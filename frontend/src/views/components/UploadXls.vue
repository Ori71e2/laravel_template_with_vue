<template>
  ...
  <el-dialog title="文件导入" center :visible="uploadDialogFormVisible" :close-on-click-modal="false" @close="cancelUpload()">
    ...
    >
      ...
      <el-button style="margin-left: 10px;" size="small" type="success" disabled @click="submitUpload">上传到服务器</el-button>
  ...
  methods: {
    ...
    downloadTemplate() {
      ...
      ({ download }) => {
        download()
          .then(res => {
            // let file = process.env.VUE_APP_BASE_API + res.info
            const blob = new Blob([res], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
            var downloadElement = document.createElement('a')
            var href = window.URL.createObjectURL(blob)
            downloadElement.href = href
            downloadElement.download = this.templateFile
